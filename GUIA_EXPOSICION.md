...
> "Utilicé **SQLAlchemy ORM** (Object-Relational Mapping). Esto me permite
> trabajar con clases de Python en lugar de escribir SQL crudo. La base de datos
> tiene 4 tablas (incluyendo `historial` y `etiquetas`) con relaciones,
> validaciones automáticas y persistencia en SQLite."

### Mostrar en el código (`src/models.py`):
...
class Tarea(Base):
    ...
    def validar_estado(self, key, valor):
        if valor not in ('pendiente', 'completada', 'vencida'):
...
1. **Clases ORM**: Cada clase (`Tarea`, `Categoria`) representa una tabla en `models.py`.
2. **Relaciones**: `relationship` y `ForeignKey` manejan la integridad referencial (1:N, N:M).
3. **Validaciones Python**: Uso de decoradores `@validates` para asegurar datos correctos (ej: estado, prioridad).
4. **Mapeo Automático**: SQLAlchemy convierte automáticamente estas clases en sentencias SQL.
5. **Creación Automática**: `Base.metadata.create_all()` en `database.py` crea las tablas al iniciar si no existen.
...
### "¿Por qué elegiste SQLAlchemy en lugar de SQL directo?"

> "Opté por SQLAlchemy porque es el estándar en la industria profesional.
> Me permite:
> 1. **Abstracción**: Trabajo con objetos Python, no con cadenas de texto SQL propensas a errores.
> 2. **Seguridad**: Previne automáticamente inyecciones SQL.
> 3. **Mantenibilidad**: Es mucho más fácil leer `session.add(tarea)` que un `INSERT INTO...`.
> 4. **Potencia**: Manejar relaciones complejas como Many-to-Many (Etiquetas) es trivial con el ORM."
